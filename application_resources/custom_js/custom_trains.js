...
$(document).ready(function() {
    //trains table
    var trains_table = $('#trains_table').dataTable({
        "sDom": "<'row'<'col-md-6'l <'toolbar trains_table_tbar'>><'col-md-6'f>r>t<'row'<'col-md-12'p i>>",
        "oTableTools": {
            "aButtons": [
                {
                    "sExtends": "collection",
    ...
    $(".trains_table_tbar").html('<div class="table-tools-actions"><button class="btn btn-primary" style="margin-left:12px" id="add_company_btn" data-toggle="modal" data-target="#add_company_modal">Add New Company</button></div>');

    $('#trains_table_wrapper .dataTables_filter input').addClass("input-medium ");
    $('#trains_table_wrapper .dataTables_length select').addClass("select2-wrapper span12");
    $(".select2-wrapper").select2({minimumResultsForSearch: -1});

    //add trains Form
    $('#add_trains_form').validate({
        focusInvalid: false,
        ignore: "",
        rules: {
            company_code: {
